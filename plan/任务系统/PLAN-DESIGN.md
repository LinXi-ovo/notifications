...
> 基于 v3 设计 + 实际实现过程中的修正与新增。覆盖已实现的功能和设计变化。
...
| **CRUD 安全** | 未区分 | **v3.2 新增：编辑模式/执行模式** — 编辑模式可增删节点/角色/边，执行模式只能按角色权限操作 |
| **管理员测试** | 无 | **v3.2 新增：adminBypass 开关** — 默认关闭，admin 也受权限约束；打开后可绕过。持久化到 localStorage |
| **权限映射** | 需手动配置 `mission.permissions` | **v3.2 新增：角色名称自动映射** — 按名称匹配 DEFAULT_PERMISSIONS（需审批角色→executor、审核员→reviewer……） |
| **权限叠加** | 单角色权限 | **v3.2 新增：用户所有已认领角色的转换叠加** — 同时有执行人(todo→进行中)和审核员(进行中→完成)则两者都可用 |
| **count/all 模式** | 显示「标记完成」 | **v3.2 修改：隐藏手动「标记完成」，只有「标记我已填写」，满人自动完成** |
| **「审核通过」标签** | reviewer 有「审核通过」转换 | **v3.2 修改：统一为「标记完成」** — 底层都是 changeNodeStatus('completed')，标签诚实化 |
| **approver 转换** | 有 completed→completed 空转换 | **v3.2 修改：移除空转换，approver 零转换** |
| **无操作提示** | 无 | **v3.2 新增：显示具体原因** — 节点已完成/前置未完成/角色在当前状态无匹配转换 |
| **调试预设** | 无 | **v3.2 新增：10 个一键加载预设**（6 任务流 + 4 权限模型）+ 权限诊断对话框 |
| **达标概念** | 无 | **v3.2 新增：count/all 模式有人提交但未满员 → 📊 达标标记，提示下游可开始** |
| **blocked 状态** | 自动依赖阻塞 | **v3.2 修正：当前为手动标记** — 用户手动标阻塞/解除，自动 DAG 依赖引擎未实现 |
...
→ 同 v3，无数据模型变化。所有 v3.2 功能在 Store 层实现，不修改 JSON schema。

### 2.2 TaskNode（任务节点）

→ 同 v3。注意：`blocked` 当前为手动标记，非自动 DAG 推导。

### 2.4 Role（角色）

→ 同 v3。注意：`claimPolicy` **仅控制谁能认领角色**，不决定认领后的权限。认领后的权限由角色名称自动映射决定。
...
**v3.2 确认**：`blocked` 为**手动标记**，用户自行判断并点击标记/解除。自动 DAG 依赖阻塞未实现。
...
### 3.1 主视图布局（MissionGraphView v3.2）
...
| **编辑模式/执行模式** | MissionGraphView 模式切换 | ✅ v3.2 |
| **adminBypass 开关** | store.state + 调试栏 UI | ✅ v3.2 |
| **角色名 → 权限映射** | _getRolePermissions + nameMap | ✅ v3.2 |
| **10 个调试预设** | mission-presets.js | ✅ v3.2 |
...
| **权限叠加** | getAllowedTransitions 用 userRoleIds 而非 matchingRoleIds | ✅ v3.2 |
| **无操作原因提示** | noOpReason computed（5 种原因） | ✅ v3.2 |
| **达标标记** | count/all 模式部分完成时显示 | ✅ v3.2 |
| **DEFAULT_PERMISSIONS 诚实化** | 移除空转换/统一 label | ✅ v3.2 |
...
1. **子图数据存储**：未实现
2. **评论通知方式**：未实现
3. **DAG 自动依赖阻塞**：v3.2 确认当前为手动 blocked，自动引擎待实现
