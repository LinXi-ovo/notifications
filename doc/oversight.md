...
1. **`mousedown` 抢焦点**：按钮 `mousedown` 默认行为让编辑器失焦 → ProseMirror 状态紊乱
2. **`type=submit` 触发表单提交**：`<button>` 在 `<form>` 内默认 `type="submit"`，点击后触发表单提交 → 页面刷新 → 编辑器销毁

两者叠加：即使编辑器扛住了失焦，表单提交也会直接炸掉编辑器。

**真实修复**：
- `type="button"` — 显式声明非提交按钮，防止触发表单提交
- `@mousedown.prevent` — 防止 mousedown 抢焦点
